...
module.exports = {
    ...
    try {
      ...
      if (!user.length) {
        ...
        let user = new User({
          ...
          uniqueId: await userHelper.generateUniqueId(payload.type),
        });
        user.password = user.encryptPassword(req.body.password);
        await user.save();
        // let token = await new jwtService().createJwtToken({ id: user._id, email: user.username });
        return response.created(res, { email: user.email, uniqueId: user.uniqueId });
      } else {
  ...
  // Called on login and whenever the app's OneSignal subscription id
  // changes, so pushes always land on the device(s) currently signed in
  // for that user. Keyed on player_id so re-registering (or a device
  // switching accounts) upserts rather than duplicating rows.
  registerDevice: async (req, res) => {
    try {
      const { user_id, player_id, device_token } = req.body;
      ...
      await Device.findOneAndUpdate(
        { player_id },
        { $set: { player_id, device_token, user: user_id } },
        { upsert: true }
      );
      return response.ok(res, { message: "Device registered." });
    } catch (error) {
      return response.error(res, error);
    }
  },
